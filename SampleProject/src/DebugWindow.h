...
public:
	...
	void OnUpdate(double deltaTime) override {
		if (Vega::Timer::FrameCount() % 100 == 0) {
			Log.trace<Vega::Color::PurpleLight>(
				"Running... time ellapsed is {:.4f}s and delta time is {:.4f}ms", Vega::Timer::TimeS(), Vega::Timer::DeltaTime()
			);
		}
	}
