...
export inline Vec3<double> Mul(const Mat3x3& M, const Vec3<double>& u) {
	Vec3<double> v;
	v.x = dot(M.L1, u);
	v.y = dot(M.L2, u);
	v.z = dot(M.L3, u);
...
export inline Mat3x3 Mul(const Mat3x3& A, const Mat3x3& B) {
	Mat3x3 M;
	M.a1 = dot(A.L1, B.C1);
	M.a2 = dot(A.L1, B.C2);
	M.a3 = dot(A.L1, B.C3);
...
export inline void Mul(const Mat3x3& A, const Mat3x3& B, Mat3x3& M) {
	if (!(&M != &A && &B != &M)) {
